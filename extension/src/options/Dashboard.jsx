...
import { fetchBotStatus } from '../utils/api';
...
const Dashboard = () => {
    ...
    useEffect(() => {
        ...
        const checkBotStatus = async () => {
            try {
                ...
                if (res) {
                    setBotState({ status: res.status || 'DISCONNECTED', qrCode: res.qrCode || null });
                }
            } catch (err) {
    ...
    const navIdleClass = darkMode ? 'text-slate-300 hover:bg-slate-800/75' : 'text-slate-700 hover:bg-white/70';

    return (
        <div className={`relative flex min-h-screen overflow-hidden ${appClass} font-sans`}>
            <div className="pointer-events-none absolute inset-0" aria-hidden="true">
                <div className={`absolute -left-24 -top-20 h-72 w-72 rounded-full blur-3xl ${darkMode ? 'bg-fuchsia-500/20' : 'bg-emerald-300/35'}`} />
                <div className={`absolute -right-30 top-30 h-80 w-80 rounded-full blur-3xl ${darkMode ? 'bg-sky-500/20' : 'bg-sky-300/35'}`} />
                <div
                    className={`absolute ${activeTab === 'inbox' ? '-bottom-35 opacity-45' : '-bottom-22.5'} left-[35%] h-72 w-72 rounded-full blur-3xl ${
                        darkMode
                            ? activeTab === 'inbox'
                                ? 'bg-cyan-500/10'
                                : 'bg-cyan-500/15'
            ...
            <aside className={`z-10 sticky top-0 h-screen self-start w-67.5 border-r backdrop-blur-2xl flex flex-col ${sidebarClass}`}>
                ...
                <div className={`space-y-3 p-4 border-t ${darkMode ? 'border-slate-700/50' : 'border-white/60'}`}>
                    {botState.status === 'AWAITING_QR' && botState.qrCode && (
                        <div className="mb-4 rounded-xl bg-white p-3 shadow-sm text-center">
                            <p className="text-xs text-slate-700 font-semibold mb-2">Escaneie para Conectar</p>
                            <img src={botState.qrCode} alt="WhatsApp QR Code" className="w-full h-auto rounded border" />
                            <p className="text-[10px] text-slate-500 mt-2">Robô no Servidor Nuvem</p>
                        </div>
                    )}
                    ...
                    <div className={`flex items-center gap-2 text-xs ${darkMode ? 'text-slate-400' : 'text-slate-600'}`}>
                        <span className={`inline-block h-2 w-2 rounded-full ${botState.status === 'LOGGED_IN' ? 'bg-emerald-400' : botState.status === 'AWAITING_QR' ? 'bg-amber-400' : 'bg-rose-400'}`} />
                        <span>Status Bot: {botState.status}</span>
                    </div>
            ...
            <main className="relative z-10 min-w-0 flex-1">
                ...
                <div className="px-10 pb-10 pt-8">{renderContent()}</div>
            </main>
        </div>
    );
};
