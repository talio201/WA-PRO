...
const NewCampaign = ({ onCancel }) => {
  ...
  const manualContacts = useMemo(
    () => parseManualContacts(manualText),
    [manualText],
  );
  useEffect(() => {
    setManualSelectedPhones((current) => {
      const currentSet = new Set(current);
      const kept = manualContacts
        .filter((item) => currentSet.has(item.phone))
        .map((item) => item.phone);
  ...
  const selectedManualContacts = useMemo(
    () =>
      manualContacts.filter((item) =>
        manualSelectedPhones.includes(item.phone),
      ),
  ...
  const loadDbContacts = useCallback(async () => {
    setLoadingDb(true);
    try {
      const dbList = await getContacts();
      const grouped = new Map();
      (Array.isArray(dbList) ? dbList : []).forEach((item) => {
        const phone = normalizePhone(item.phone);
  ...
  const handleSubmit = async () => {
    ...
    if (finalContacts.length === 0)
      return alert("Selecione ao menos um contato.");
          ...
          <section className="new-campaign-main space-y-8 p-7 xl:p-10">
            <header className="flex flex-wrap items-start justify-between gap-4">
              <div>
                ...
                <p className="mt-2 text-base text-slate-600">
                  Fluxo único e intuitivo para montar, variar e disparar sua
                  campanha.
                </p>
              </div>
              <button
            ...
            <div className="grid gap-4 md:grid-cols-[minmax(0,1fr)_130px_130px_130px]">
              ...
              <div className="new-campaign-kpi rounded-xl border border-slate-200 bg-white px-3 py-2">
                ...
                <p
                  className={`mt-1 text-2xl font-bold ${turboMode ? "text-emerald-700" : "text-slate-900"}`}
                >
                  {turboMode ? "ON" : "OFF"}
                </p>
              </div>
                    ...
                    <div className="mt-3 flex items-center justify-between text-sm text-slate-600">
                      ...
                      <div className="space-x-3">
                        <button
                          type="button"
                          onClick={() =>
                            setManualSelectedPhones(
                              manualContacts.map((item) => item.phone),
                            )
                          }
                          className="new-campaign-link-btn font-semibold text-emerald-700"
                        >
                          Selecionar todos
                        </button>
                        <button
                          type="button"
                          onClick={() => setManualSelectedPhones([])}
                          className="new-campaign-link-btn font-semibold text-slate-500"
                        >
                          Limpar
                        </button>
                      </div>
                  ...
                  <>
                    ...
                    >
                      ...
                      {excelFile
                        ...
                        : "Arraste ou clique para importar Excel/CSV"}
                    </div>
                    <div className="mt-3 flex items-center justify-between text-sm text-slate-600">
                      <span>
                        {excelContacts.length} contato(s) importado(s)
                      </span>
                      <div className="space-x-3">
                        <button
                          type="button"
                          onClick={() =>
                            setExcelSelectedPhones(
                              excelContacts.map((item) => item.phone),
                            )
                          }
                          className="new-campaign-link-btn font-semibold text-emerald-700"
                        >
                        ...
                        <button
                          type="button"
                          onClick={() => setExcelSelectedPhones([])}
                          className="new-campaign-link-btn font-semibold text-slate-500"
                        >
                          Limpar
                        </button>
                  ...
                  <div className="mt-3 space-y-2">
                    {source === "manual" &&
                      manualContacts.map((contact) => (
                        <label
                          key={`m-${contact.phone}`}
                          className="new-campaign-contact-row flex items-center gap-2 rounded-lg px-2 py-1.5 text-sm"
                        >
                          <input
                            type="checkbox"
                            checked={manualSelectedPhones.includes(
                              contact.phone,
                            )}
                            onChange={(event) => {
                    ...
                    {source === "excel" &&
                      excelContacts.map((contact) => (
                        ...
                        >
                          <input
                            type="checkbox"
                            checked={excelSelectedPhones.includes(
                              contact.phone,
                            )}
                            onChange={(event) => {
                              const checked = event.target.checked;
                              setExcelSelectedPhones((current) =>
                                checked
                                  ? [...current, contact.phone]
                                  : current.filter(
                                      (phone) => phone !== contact.phone,
                    ...
                    {source === "database" &&
                      filteredDbContacts.map((contact) => (
                        ...
                        >
                          <input
                            type="checkbox"
                            checked={selectedDbContacts.some(
                              (item) => item.phone === contact.phone,
                            )}
                            onChange={(event) => {
                              const checked = event.target.checked;
                              setSelectedDbContacts((current) =>
                                checked
                                  ? [...current, contact]
                                  : current.filter(
                                      (item) => item.phone !== contact.phone,
                  ...
                  <div className="flex items-center justify-between bg-[#075E54] px-4 py-3 text-white">
                    <div className="min-w-0">
                      <p className="truncate text-base font-semibold">
                        {previewContact.name || "Cliente"}
                      </p>
                      <p className="truncate text-xs text-emerald-100">
                        {formatPhonePreview(previewContact.phone)}
                      </p>
                    </div>
                ...
                <div className="mt-3 grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <input
                      ...
                      onChange={(event) =>
                        setMinDelaySeconds(event.target.value)
                      }
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(event) =>
                        setMaxDelaySeconds(event.target.value)
                      }
                      className="new-campaign-input w-full rounded-xl border border-slate-300 px-3 py-2 text-sm outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100"
                    />
                  </div>
