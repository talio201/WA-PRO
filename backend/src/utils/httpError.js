...
function classifyStorageError(rawMessage) {
  ...
  if (message.includes("permission denied")) {
    return {
      ...
      message:
        "Supabase permission denied. Configure SUPABASE_SERVICE_ROLE_KEY in backend/.env.",
    };
  }
  if (message.includes("could not find the table")) {
    return {
      ...
      message:
        "Supabase table not found. Execute backend/supabase/schema.sql or backend/supabase/add_conversation_assignments.sql in Supabase SQL Editor.",
    };
  }
  if (message.includes("invalid api key")) {
    return {
      ...
      message:
        "Invalid Supabase key. Verify SUPABASE_SERVICE_ROLE_KEY or SUPABASE_ANON_KEY in backend/.env.",
    };
  }
  return null;
}
