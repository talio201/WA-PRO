...
function buildSearchTerms(
  ...
  terms.add(`+${normalizedDigits}`);
  if (
    normalizedDigits.startsWith(defaultCountryCode) &&
    normalizedDigits.length > defaultCountryCode.length
  ) {
...
function normalizePhone(rawPhone, options = {}) {
  const defaultCountryCode =
    String(options.defaultCountryCode || DEFAULT_COUNTRY_CODE).replace(
      /\D/g,
      "",
  ...
  if (
    (digits.length === 10 || digits.length === 11 || digits.length === 12) &&
    digits.startsWith("0")
  ) {
  ...
  const isValid =
    digits.length >= MIN_PHONE_LENGTH && digits.length <= MAX_PHONE_LENGTH;
  return {
    raw,
    normalized: digits,
    isValid,
