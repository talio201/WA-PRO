...
function setNestedValue(item, pathExpression, value) {
  const keys = String(pathExpression || "")
    .split(".")
    .filter(Boolean);
...
function sortItems(items, sortSpec = {}) {
  ...
  return [...items].sort((aItem, bItem) => {
    const aValue = field.includes(".")
      ? getNestedValue(aItem, field)
      : aItem[field];
    const bValue = field.includes(".")
      ? getNestedValue(bItem, field)
      : bItem[field];
...
function getSupabaseClient() {
  ...
  if (!serviceRoleKey && anonKey && !hasWarnedSupabaseAnonOnly) {
    ...
    console.warn(
      "SUPABASE_SERVICE_ROLE_KEY is missing. Backend is using SUPABASE_ANON_KEY and may fail with permission denied.",
    );
  }
...
class LocalDB {
  ...
  _buildSupabaseQuery(baseQuery, query = {}) {
    ...
    Object.entries(query || {}).forEach(([field, expected]) => {
      ...
      if (
        ...
        const values = (Array.isArray(expected.$in) ? expected.$in : []).map(
          (value) => stripFunctionsDeep(value),
        );
  ...
  async _readCollection(query = {}) {
    if (!this._isSupabaseEnabled()) {
      ...
      const collection = Array.isArray(db[this.collection])
        ? db[this.collection]
        : [];
    ...
    if (error) {
      throw new Error(
        `Supabase read error (${this.collection}): ${error.message}`,
      );
    }
    return (Array.isArray(data) ? data : []).map((row) =>
      mapFromDb(this.collection, row),
    );
  }
  find(query = {}) {
  ...
  async findById(id) {
    ...
    if (error) {
      throw new Error(
        `Supabase findById error (${this.collection}): ${error.message}`,
      );
    }
  ...
  async create(data) {
    ...
    if (error) {
      throw new Error(
        `Supabase create error (${this.collection}): ${error.message}`,
      );
    }
  ...
  async insertMany(items = []) {
    ...
    if (error) {
      throw new Error(
        `Supabase insertMany error (${this.collection}): ${error.message}`,
      );
    }
    return (Array.isArray(inserted) ? inserted : []).map((row) =>
      mapFromDb(this.collection, row),
    );
  }
  async findByIdAndUpdate(id, update) {
    ...
    if (error) {
      throw new Error(
        `Supabase findByIdAndUpdate error (${this.collection}): ${error.message}`,
      );
    }
  ...
  async findOneAndUpdate(query, update, options = {}) {
    ...
    const normalizedSort =
      rawSort &&
      Object.keys(rawSort).length === 1 &&
      Object.prototype.hasOwnProperty.call(rawSort, "_id")
        ? { createdAt: rawSort._id }
        : rawSort;
    const sorted = normalizedSort
      ? sortItems(matches, normalizedSort)
      : matches;
  ...
  async deleteMany(query = {}) {
    ...
    if (error) {
      throw new Error(
        `Supabase deleteMany error (${this.collection}): ${error.message}`,
      );
    }
